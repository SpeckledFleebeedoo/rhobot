...
pub async fn clear_server_data(server_id: i64, db: &Pool<Sqlite>) -> Result<(), DatabaseError> {
    sqlx::query!(r#"DELETE FROM servers WHERE server_id = $1"#, server_id)
        ...
        .await?;
    sqlx::query!(
        r#"DELETE FROM subscribed_mods WHERE server_id = $1"#,
        server_id
    )
    .execute(db)
    .await?;
    sqlx::query!(
        r#"DELETE FROM subscribed_authors WHERE server_id = $1"#,
        server_id
    )
    .execute(db)
    .await?;
...
pub async fn get_server_faqs(
    server_id: i64,
    db: &Pool<Sqlite>,
) -> Result<HashMap<String, Vec<String>>, DatabaseError> {
    let db_entries = sqlx::query!(
        r#"SELECT title, link FROM faq WHERE server_id = $1"#,
        server_id
    )
    .fetch_all(db)
    .await?;
    let mut faq_map: HashMap<String, Vec<String>> = HashMap::new();
    let base_faqs: Vec<String> = db_entries
        .iter()
        .filter(|f| f.link.is_none())
        .map(|f| f.title.clone())
        .collect();
    let link_faqs: Vec<(String, String)> = db_entries
        .iter()
        .filter_map(|f| f.link.clone().map(|l| (f.title.clone(), l)))
        .collect();
...
pub async fn get_server_faq_dump(
    db: &Pool<Sqlite>,
    server_id: i64,
) -> Result<Vec<BasicFaqEntry>, DatabaseError> {
    let server_faqs = sqlx::query_as!(
        BasicFaqEntry,
        r#"SELECT title, contents, image, link FROM faq WHERE server_id = $1"#,
        server_id
    )
    .fetch_all(db)
    .await?;
...
pub async fn delete_faq_entry(
    ...
    Ok(sqlx::query!(
        r#"DELETE FROM faq WHERE server_id = $1 AND (title = $2 OR link = $2)"#,
        server_id,
        name
    )
    .execute(db)
...
pub async fn find_faq_entry_opt(
    ...
    server_id: i64,
    name: &str,
) -> Result<Option<BasicFaqEntry>, DatabaseError> {
    Ok(sqlx::query_as!(
        BasicFaqEntry,
...
pub async fn get_modrole(db: &Pool<Sqlite>, server_id: i64) -> Result<Option<i64>, DatabaseError> {
    let role = sqlx::query!(
        r#"SELECT modrole FROM servers WHERE server_id = $1"#,
        server_id
    )
    .fetch_one(db)
    .await?
...
pub async fn get_server_info(
    db: &Pool<Sqlite>,
    server_id: i64,
) -> Result<Option<DBServerInfo>, DatabaseError> {
    let serverdata = sqlx::query_as!(
        DBServerInfo,
        r#"SELECT * FROM servers WHERE server_id = $1"#,
        server_id
    )
    .fetch_optional(db)
    .await?;
...
pub async fn get_subscribed_mods(
    ...
    let subscribed_mods = sqlx::query!(
        r#"SELECT mod_name FROM subscribed_mods WHERE server_id = $1"#,
        server_id
    )
    .fetch_all(db)
    .await?
...
pub async fn get_subscribed_authors(
    ...
    let subscribed_authors = sqlx::query!(
        r#"SELECT author_name FROM subscribed_authors WHERE server_id = $1"#,
        server_id
    )
    .fetch_all(db)
    .await?
...
pub async fn store_updates_channel(
    db: &Pool<Sqlite>,
    server_id: i64,
    channel_id: i64,
) -> Result<(), DatabaseError> {
        ...
        .is_some()
    {
        ...
        sqlx::query!(
            r#"UPDATE servers SET updates_channel = $1 WHERE server_id = $2"#,
            channel_id,
            server_id
        )
        .execute(db)
        ...
        sqlx::query!(
            r#"INSERT INTO servers (server_id, updates_channel) VALUES ($1, $2)"#,
            server_id,
            channel_id
        )
        .execute(db)
...
pub async fn store_modrole(
        ...
        .is_some()
    {
        // Update server data if it does exist
        sqlx::query!(
            r#"UPDATE servers SET modrole = $1 WHERE server_id = $2"#,
            role_id,
            server_id
        )
        .execute(db)
        ...
        // Add server and set setting if it does not exist
        sqlx::query!(
            r#"INSERT INTO servers (server_id, modrole) VALUES ($1, $2)"#,
            server_id,
            role_id
        )
        .execute(db)
...
pub async fn store_changelog_setting(
    db: &Pool<Sqlite>,
    server_id: i64,
    show_changelogs: bool,
) -> Result<(), DatabaseError> {
    match sqlx::query!(
        r#"SELECT server_id FROM servers WHERE server_id = $1"#,
        server_id
    )
    .fetch_optional(db)
    .await?
    {
        Some(_) => {
            // Update server data if it does exist
            sqlx::query!(
                r#"UPDATE servers SET show_changelog = $1 WHERE server_id = $2"#,
                show_changelogs,
                server_id
            )
            .execute(db)
        ...
        None => {
            // Add server and set setting if it does not exist
            sqlx::query!(
                r#"INSERT INTO servers (server_id, show_changelog) VALUES ($1, $2)"#,
                server_id,
                show_changelogs
            )
            .execute(db)
...
pub async fn add_mod_subscription(
    db: &Pool<Sqlite>,
    server_id: i64,
    modname: &str,
) -> Result<(), DatabaseError> {
    sqlx::query!(
        r#"INSERT OR REPLACE INTO subscribed_mods (server_id, mod_name) VALUES ($1, $2)"#,
        server_id,
        modname
    )
    .execute(db)
...
pub async fn remove_mod_subscription(
    db: &Pool<Sqlite>,
    server_id: i64,
    modname: &str,
) -> Result<(), DatabaseError> {
    sqlx::query!(
        r#"DELETE FROM subscribed_mods WHERE server_id = $1 AND mod_name = $2"#,
        server_id,
        modname
    )
    .execute(db)
...
pub async fn add_author_subscription(
    db: &Pool<Sqlite>,
    server_id: i64,
    author: &str,
) -> Result<(), DatabaseError> {
    sqlx::query!(
        r#"INSERT OR REPLACE INTO subscribed_authors (server_id, author_name) VALUES ($1, $2)"#,
        server_id,
        author
    )
    .execute(db)
...
pub async fn remove_author_subscription(
    db: &Pool<Sqlite>,
    server_id: i64,
    author: &str,
) -> Result<(), DatabaseError> {
    sqlx::query!(
        r#"DELETE FROM subscribed_authors WHERE server_id = $1 AND author_name = $2"#,
        server_id,
        author
    )
    .execute(db)
...
pub async fn get_last_mod_update_time(
    db: &Pool<Sqlite>,
    modname: &str,
...
pub async fn store_mod_data(
    db: &Pool<Sqlite>,
    mod_details: DBModEntry<'_>,
) -> Result<(), DatabaseError> {
...
pub async fn create_subscriptions_cache(
    db: &Pool<Sqlite>,
) -> Result<Vec<SubCacheEntry>, DatabaseError> {
    let mod_records = sqlx::query!(r#"SELECT * FROM subscribed_mods"#)
        .fetch_all(db)
        .await?
        .iter()
        .map(|rec| SubCacheEntry {
