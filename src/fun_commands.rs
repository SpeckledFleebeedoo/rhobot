...
fn time_left_message() -> String {
    ...
    let minutes =
        (time_to_anniversary - TimeDelta::days(days) - TimeDelta::hours(hours)).num_minutes();
    let seconds = (time_to_anniversary
        - TimeDelta::days(days)
        - TimeDelta::hours(hours)
        - TimeDelta::minutes(minutes))
    .num_seconds();
    format!(
        "Space Age anniversary is in {days} days, {hours} hours, {minutes} minutes and {seconds} seconds!"
    )
}
...
pub fn time_until_anniversary(year: i32) -> chrono::TimeDelta {
    ...
    let prague_anniversary_datetime = Prague
        .from_local_datetime(&NaiveDateTime::new(anniversary_date, anniversary_time))
        .unwrap();
