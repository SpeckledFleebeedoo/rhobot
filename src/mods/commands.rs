...
use super::{
    error::ModError,
    search_api,
    update_notifications::{SubCacheEntry, SubscriptionType},
};
...
pub async fn show_changelogs(ctx: Context<'_>, show_changelogs: bool) -> Result<(), Error> {
...
#[poise::command(
    ...
    guild_only,
    check = "is_mod",
    subcommands("unsubscribe_author", "unsubscribe_mod"),
    subcommand_required,
...
#[poise::command(
    ...
    guild_only,
    check = "is_mod",
    subcommands("subscribe_author", "subscribe_mod"),
    subcommand_required,
    category = "Subscriptions"
)]
...
pub async fn show_subscriptions(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let subscribed_mods_vec = database::get_subscribed_mods(db, server_id).await?;
    ...
    let subscribed_authors_vec = database::get_subscribed_authors(db, server_id).await?;
