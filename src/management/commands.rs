...
pub async fn reset_server_settings(ctx: Context<'_>) -> Result<(), Error> {
    let server_id = get_server_id(ctx)?;
    let db = &ctx.data().database;
    database::clear_server_data(server_id, db).await?;
    ctx.reply("Server data reset").await?;
    Ok(())
}
...
pub async fn get_server_info(ctx: Context<'_>) -> Result<(), Error> {
    let server_id = get_server_id(ctx)?;
    ...
    match serverdata {
        Some(data) => {
            let updates_channel = data
                .updates_channel
                .map_or_else(|| "Not set".to_owned(), |ch| format!("<#{ch}>"));
            let modrole = data
                .modrole
                .map_or_else(|| "Not set".to_owned(), |role| format!("<@&{role}>"));
            let show_changelog = data
                .show_changelog
                .map_or_else(|| "Not set (default to true)".to_owned(), |b| b.to_string());
            let response = format!(
                "**Stored information for this server:**\nServer ID: {:?}\nUpdates channel: {}\nmodrole: {}\nShow changelogs: {}",
